...
**本地 Project 命名基线（2026-07-13）：** `Qn·` 季度前缀（`Q1·职业事业 / Q2·财富投资 / Q4·智力技术`）作为个人分类标签**保留**——它们只是静态标签，与远端 KR/OKR 无任何实时绑定，看着像 OKR 但机制上已解耦。2026-03-30 迁移遗留的 `🗑️ 冷冻舱`（旧 `General Tasks` 催化桶）已删除：本地无需"冷冻区"，其 1 任务 / 36 会话按用户决定整清（备份后操作，详见 `docs/devlog/2026-07-13.md` 第 7 节）。造出该桶、且内嵌旧 KR→Project 模型的一次性脚本 `scripts/migrate_projects.py` 同步退役。

仍然不要恢复 PocketBase 或旧 `WEEKLY_BRIDGE` 任务接口。

## 工程规范基线（2026-07-12）

在功能路线推进的同时，仓库已完成一次结构规范化，作为后续所有阶段的工程基线（详细过程见 `docs/devlog/2026-07-12.md` 第 5 节）：

- **源码布局**：所有应用模块位于 `src/focusflow/` 包内；包内互相引用一律使用绝对导入 `from focusflow.<module> import ...`，禁止再用旧的顶层裸导入（`import ui_manager`）。新增模块直接放进该包。
- **入口与启动**：运行入口是 `main()`；通过 `python -m focusflow`（`PYTHONPATH=src`）或根目录 `Pomodoro*.bat` 启动。启动器必须继续使用 `.venv\Scripts\pythonw.exe`（受 `test_launch_contract` 约束）。
- **资源与数据**：`images/`、`sound/` 为随包分发的静态资源置于项目根；可变用户数据仍由 `AppPaths` 落到 `%LOCALAPPDATA%\FocusFlow`。任何 `__file__` 相对定位新增代码时，需注意包已下沉两级（项目根 = `parents[2]`）。
- **打包与测试**：包元数据与 `pytest` 配置集中在 `pyproject.toml`（`pythonpath=src`）；`FocusFlow.spec` 为受跟踪的 PyInstaller 配方（入口 `src/focusflow/main.py`）。手动/GUI 测试通过 `tests/conftest.py` 跳过。
- **维护脚本**：一次性数据迁移、报表重生成等脚本归入 `scripts/`，不与应用包混放。
- **清理约束**：不再向仓库提交一次性巡检/调试脚本、实验代码、缓存或运行时状态；此类文件应留在本地并由 `.gitignore` 覆盖。
